...
public class ProviderController {
    ...
    @Operation(summary = "Update provider profile", description = "Upload/update provider documents and select services. Only PROVIDER allowed.")
    public ResponseEntity<ApiResponse<ProviderProfileResponseDTO>> updateProfile(
        ...
        return ResponseEntity.ok(providerService.updateProfile(documentFile, selectedServices));
    }

    // Download provider document
    @GetMapping("/document")
    @PreAuthorize("hasRole('PROVIDER')")
    @Operation(summary = "Download provider document", description = "Allows provider to download their uploaded verification document")
    public ResponseEntity<byte[]> downloadDocument() {

        ProviderProfile p = providerService.getProviderDocument();

        return ResponseEntity.ok()
                .header("Content-Disposition", "attachment; filename=\"" + p.getDocumentName() + "\"")
    ...
    @PutMapping("/status")
    @PreAuthorize("hasRole('PROVIDER')")
    @Operation(summary = "Update provider availability status", description = "Provider can update their current status (ACTIVE, INACTIVE, etc.)")
    ...
    @Operation(summary = "Approve provider (ADMIN)", description = "Admin approves a provider after verification")
    public ResponseEntity<ApiResponse<ProviderProfileResponseDTO>> approveProvider(@PathVariable Long id) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Reject provider (ADMIN)", description = "Admin rejects a provider application")
    public ResponseEntity<ApiResponse<ProviderProfileResponseDTO>> rejectProvider(@PathVariable Long id) {
        return ResponseEntity.ok(providerService.rejectProvider(id));
    }

    // Pending providers for admin
    @GetMapping("/pending")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Get pending providers (ADMIN)", description = "Fetch paginated list of providers awaiting approval")
